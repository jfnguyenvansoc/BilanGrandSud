...
arcpy.AddMessage("Le gros matou recupere les informations des stations et parametres du Bilan Grand Sud")
Params = [[p[0],p[1],p[2],p[3],p[4]] for p in arcpy.da.SearchCursor(TB_PARAMETRES,["id_parametre", "nom", "BilanENV", "unite", "BilanEnvSeuil", "OrdreBilanENV"], "id_parametre LIKE '%/" + IdSuivi + "/%' AND BilanENV = 'Oui'", sql_clause=(None,'ORDER BY OrdreBilanENV'))]
...
if len(Params) == 0:
    ...
    if IdSuivi == "SURF":
        ...
        SQLStation = "BilanEnvCompartiment LIKE '%" + IdSuivi + "%' AND BilanENVOrdre > 0 OR BilanEnvCompartiment LIKE '%" + IdSuivi + "%' AND BilanENVOrdreRef > 0"
    else:
                ...
                for p in Params:
                    ...
                    if len(DatasSelDateP) == 0:
                        ...
                        while a < nbdt:
                            ...
                            #Condition de division par 2 si < LQ
                            if IdSuivi == "SURF":
                               if DatasSelDateP[a][6] == "<":
                                   OtherDatasS.append([a, y, y + 9, DatasSelDateP[a][6]])
                                   OtherDatasM.append([a, y, y + 9, DatasSelDateP[a][4]])
                                   if DatasSelDateP[a][5].find(",") > 0:
                                       v = DatasSelDateP[a][5].replace(",",".")
                                   else:
                                       v = DatasSelDateP[a][5]
                                   v = float(v) / 2
                                   v = str(v).replace(".", ",")
                                   OtherDatas.append([a, y, y + 9, v])
                               else:
                                   OtherDatasS.append([a, y, y + 9, DatasSelDateP[a][6]])
                                   OtherDatasM.append([a, y, y + 9, DatasSelDateP[a][4]])
                        ...
                        # Condition de division par 2 si < LQ pour SURF
                        if IdSuivi == "SURF":
                            DatasSelDateP = DatasSelDateP[0]
                            if DatasSelDateP[6] == "<":
                                LineM.append(DatasSelDateP[4])
                                LineS.append(DatasSelDateP[6])
                                if DatasSelDateP[5].find(",") > 0:
                                    v = DatasSelDateP[5].replace(",",".")
                                else:
                                    v = DatasSelDateP[5]
                                v = float(v) / 2
                                v = str(v).replace(".", ",")
